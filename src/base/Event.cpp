...
bool nuiEventTargetBase::OnEvent(const nuiEvent& rEvent)
{
  ...
  if (it_source != mpLinks.end())
  {
    ...
    {
      Link* pLink = (*it);
      DelegateMemento pFunc = pLink->mTargetFunc;
...
bool nuiEventTargetBase::CallEvent(void* pTarget, DelegateMemento pFunc, const nuiEvent& rEvent)
{
  Delegate del;
...
void nuiEventTargetBase::Connect(nuiEventSource& rSource, const DelegateMemento& rTargetFunc, void* pUser)
{
  Link* pLink = new Link;
...
void nuiEventTargetBase::Disconnect(const DelegateMemento& rTFunc)
{
...
void nuiEventTargetBase::Disconnect(nuiEventSource& rSource, const DelegateMemento& rTFunc)
{
  LinksMap::iterator it_source = mpLinks.find(&rSource);
  if (it_source != mpLinks.end())
  {
